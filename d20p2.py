...
cells_to_check = []
heapq.heappush(cells_to_check, (1, END))
score_grid[END[0]][END[1]] = 1

while len(cells_to_check) > 0:
    score, cell_to_check = heapq.heappop(cells_to_check)
    for direction in [(-1,0),(0,1),(1,0),(0,-1)]:
        ...
        if is_valid(new_cell_coordinate[0], new_cell_coordinate[1]) and score_grid[new_cell_coordinate[0]][new_cell_coordinate[1]] == 0:
            score_grid[new_cell_coordinate[0]][new_cell_coordinate[1]] = score + 1
            heapq.heappush(cells_to_check, (score+1, new_cell_coordinate))
    ...
    for x in range(n+1):
        for y in range(-n+x, n-x+1):
            if is_valid(current_coordinate[0]+x, current_coordinate[1]+y):
                valid_coordinates.add((current_coordinate[0]+x, current_coordinate[1]+y, x+abs(y)-1))
            if is_valid(current_coordinate[0]-x, current_coordinate[1]+y):
                valid_coordinates.add((current_coordinate[0]-x, current_coordinate[1]+y, x+abs(y)-1))
    return valid_coordinates

for coordinate in path:
    for after_cheat_coordinate in get_n_steps(coordinate, 20):
        if score_grid[after_cheat_coordinate[0]][after_cheat_coordinate[1]] != "#":
            if (score_grid[coordinate[0]][coordinate[1]]-after_cheat_coordinate[2]) - score_grid[after_cheat_coordinate[0]][after_cheat_coordinate[1]] >= 100:
                more_than_hundred.append(after_cheat_coordinate)
